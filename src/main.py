...
def main():
    ...
    if run_attack==True:
        if backdoor_type == 'adaptive':
            if args.load_or_train_generator == 'train':
                ...
                arguments += ['--attack_target_label',attack_target_label, '--dataset',dataset, '--poison_rate',poison_rate,
        ...
        arguments += ['--attack_target_label',attack_target_label, '--backdoor_type',backdoor_type, '--dataset',dataset, '--ER_graph_P', ER_graph_P,
                       '--poison_rate',poison_rate, '--graph_type',graph_type, '--model_hyp_set',model_hyp_set, '--PA_graph_K',PA_graph_K,  
    ...
    if run_explain==True:
        assert os.path.exists(f'{src_dir}/detect/run_explain_and_detect.py')
        arguments = ['python',f'{src_dir}/detect/run_explain_and_detect.py']
        arguments += ['--attack_target_label',attack_target_label, '--backdoor_type',backdoor_type, '--dataset',dataset, '--edge_reduction',edge_reduction, 
                      '--edge_size',edge_size, '--edge_ent',edge_ent, '--ER_graph_P',ER_graph_P, '--explain_lr',explain_lr, '--explainer_epochs',explainer_epochs, 
                      '--explanation_type',explanation_type, '--poison_rate',poison_rate, '--gen_rounds',gen_rounds,  '--graph_type',graph_type, 
                      '--model_hyp_set',model_hyp_set, '--node_feat_ent',node_feat_ent, '--node_feat_reduction', node_feat_reduction, '--node_feat_size',node_feat_size,
                      '--PA_graph_K',PA_graph_K, '--SW_graph_K',SW_graph_K, '--SW_graph_P',SW_graph_P, '--seed',seed,  '--thresh_type',thresh_type, 
                      '--thresh_val',thresh_val, '--trigger_size',trigger_size, '--lower_thresh_percentile',lower_thresh_percentile,  '--upper_thresh_percentile',upper_thresh_percentile]
        if plot==True:
            arguments += ['--plot']
        if regenerate_data==True:
            arguments += ['--regenerate_data']
