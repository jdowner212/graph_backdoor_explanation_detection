...
def poison_data_random_attack(dataset, attack_specs=None, seed=2575, verbose=False, clean_pyg_process=True, use_edge_attr=False):
    ...
    print("label distribution in backdoor train data:", count_class_samples(train_backdoor_graphs, num_classes))
...
def get_dataloader_dict(dataset_dict_backdoor, dataset_dict_clean, model_specs, classifier_hyperparams):
    ...
    # with open(f'{src_dir}/dataloader_dict.pkl','wb') as f:
...
#    elif contin_or_scratch=='continuous':
    num_node_features = train_dataset[0].x.shape[1]
    ...
    for round in range(rounds):
        ...
        print('\nTraining' if round==0 else '\nRetraining', + 'generator: iteration',round+1)
        ...
        trigger_generator,generator_optimizer = train_generator_exp_only(train_dataset_for_generator, 
                                                                         ...
                                                                         #contin_or_scratch=contin_or_scratch,
...
def train_generator_exp_only(dataset_to_attack, surrogate_model, save_path, target_label, generator_kwargs,# contin_or_scratch='continuous', 
                             trigger_generator=None, generator_optimizer=None):#, min_loss=None):
    ''' General setup '''
